...
for i in range(10):
    for j in range(10):
        ...
        plt.xlabel(r'$\ell$', fontdict=font)
        plt.ylabel('$\Delta C_\ell^{\mathrm{TT}} / C^{\mathrm{TT}}_{\ell\Lambda\mathrm{CDM}}$', fontdict=font)
        plt.title(r'TT residuals, $\Omega_{{dcdm, ini}}$ = {:.2f}, $\Omega_{{dr, ini}}$ = {:.2f} $\Gamma = 10^{{6}}$ km/s/Mpc'.format(dcdm_table[i], dcdm_table[j]), **fontdict)

        plt.savefig('plots/bf_TT_{}.pdf'.format(cnt), bbox_inches="tight")
        plt.close()
        ...
        plt.xlabel(r'$\ell$', fontdict=font)
        plt.ylabel('$\Delta C_\ell^{\mathrm{EE}} / C^{\mathrm{EE}}_{\ell\Lambda\mathrm{CDM}}$', fontdict=font)
        plt.title(r'EE residuals, $\Omega_{{dcdm, ini}}$ = {:.2f}, $\Omega_{{dr, ini}}$ = {:.2f} $\Gamma = 10^{{6}}$ km/s/Mpc'.format(dcdm_table[i], dcdm_table[j]), **fontdict)

        plt.savefig('plots/bf_EE_{}.pdf'.format(cnt), bbox_inches="tight")
        plt.close()
        cnt += 1
